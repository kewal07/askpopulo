...
s_a[175] = "Balochistan|Federally Administered Tribal Areas|Islamabad|North-West Frontier Province|Punjab|Sindh|Khyber Pakhtunkhwa|Azad Kashmir|Gilgit-Baltistan";
...
s_a[245] = "Blaenau Gwent|Bridgend|Caerphilly|Cardiff|Carmarthenshire|Ceredigion|Conwy|Denbighshire|Flintshire|Gwynedd|Isle of Anglesey|Merthyr Tydfil|Monmouthshire|Neath Port Talbot|Newport|Pembrokeshire|Powys|Rhondda Cynon Taff|Swansea|Vale of Glamorgan, The|Torfaen|Wrexham";
...
function populateCountry(elemId){
	// console.log("populateCountry",elemId);
	for (var i = 0; i < countryList.length; i++) {
		if(countryList[i].length > 0){
			var tempOption = '<option value="'+countryList[i]+'">'+countryList[i]+'</option>';
			// console.log(tempOption);
			$(elemId).append(tempOption);
		}
	}
}
function populateState(elemId,selectedCountry){
	// console.log("populateState",elemId,selectedCountry);
	$(elemId)[0].innerHTML = '<option value="nochoice">State</option>';
	if(selectedCountry != "nochoice"){
		var selectedCountryIndex = countryList.indexOf(selectedCountry);
